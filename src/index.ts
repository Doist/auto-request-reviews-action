...
async function run(): Promise<void> {
  try {
    // Get inputs
    const desiredReviewers = parseInt(core.getInput('reviewers', { required: true }));
    ...
    if (neededReviewers === 0) {
      core.info(`PR already has ${existingReviewers} reviewer(s) requested. No additional reviewers needed.`);
      return;
    }

    core.info(`PR has ${existingReviewers} reviewer(s) requested. Need to request ${neededReviewers} more.`);
    ...
    const reviewersToRequest = shuffledReviewers.slice(0, actualReviewers).map(user => user.login);
